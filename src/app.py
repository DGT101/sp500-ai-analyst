...
from data_pipeline import fetch_stock_history, calculate_technical_indicators, fetch_fundamental_metrics
...
st.caption("Uma ferramenta integrada de Machine Learning, NLP (Groq Llama-3) e Engenharia de Dados para Portfólio.")
...
SP500_TICKERS = ["AAPL", "MSFT", "GOOGL", "AMZN", "META", "NVDA", "TSLA", "BRK-B", "JNJ", "V"]
ticker = st.sidebar.selectbox("Selecione o Ativo (S&P 500)", SP500_TICKERS)

# Definição de datas pelo usuário
data_alvo = st.sidebar.date_input("Data de Referência", datetime.today() - timedelta(days=1))

# Perfil do Investidor (para calibrar o conselho da LLM)
perfil = st.sidebar.selectbox(
    "Perfil do Investidor / Objetivo",
    ["Construção de Patrimônio", "Foco em Dividendos/Renda Passiva", "Conservador/Proteção"]
)
        ...
        if not df_raw.empty:
            ...
            try:
                ...
                if target_str in df_technical.index:
                    ...
                    st.warning(f"Os mercados estavam fechados na data selecionada. Exibindo dados do último dia útil disponível.")
                ...
                with col1:
                    ...
                    metric_col1.metric("Preço de Fechamento", f"${tech_dict['Close']:.2f}")
                    metric_col2.metric("RSI (14)", f"{tech_dict['RSI']:.1f}")
                    metric_col3.metric("Div. Yield", f"{fundamentals.get('dividend_yield', 0):.2f}%")
            ...
            st.error("Não foi possível carregar dados para o ticker informado. Verifique se o ticker está correto ou tente um período diferente.")
else:
    # Estado inicial amigável quando o usuário entra na tela
    st.info("👈 Selecione os parâmetros na barra lateral esquerda e clique em 'Gerar Análise Completa' para iniciar.")
